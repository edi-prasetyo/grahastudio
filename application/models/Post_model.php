...
class Post_model extends CI_Model
{
  ...
  {
    $this->db->select('post.*,category.category_name,category_slug, user.user_name, user.user_image');
    $this->db->from('post');
    // Join
    $this->db->join('category', 'category.id = post.category_id', 'LEFT');
    $this->db->join('user', 'user.id = post.user_id', 'LEFT');
  ...
  {
    $this->db->select('post.*,category.category_name, user.user_name, user.user_image');
    $this->db->from('post');
    // Join
    $this->db->join('category', 'category.id = post.category_id', 'LEFT');
    $this->db->join('user', 'user.id = post.user_id', 'LEFT');
  ...
  public function post_popular_category($category_id)
  {
    $this->db->select('post.*,category.category_name, user.user_name');
    $this->db->from('post');
    // Join
    ...
    $this->db->join('user', 'user.id = post.user_id', 'LEFT');
    //End Join
    $this->db->where(['post_status'     =>  'Publish', 'category_id' => $category_id]);
    $this->db->order_by('post.post_views', 'ASC');
    $this->db->limit(3);
    $query = $this->db->get();
    return $query->result();
